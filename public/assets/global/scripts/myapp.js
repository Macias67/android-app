...
var MyApp = function () {
	...
	return {
		initAjax        : function (url, data, success) {
			$.ajax({
				...
				beforeSend: function (jqXHR, settings) {
					MyApp.showLoader('#00fff2');
				},
				error     : function (jqXHR, textStatus, error) {
					MyApp.playSoundError();
					MyApp.removeLoader(500);
				},
				statusCode: {
		...
		playSoundSuccess: function () {
			...
			audioElement.setAttribute('src', App.getDomain() + 'assets/global/sounds/success.mp3');
			audioElement.setAttribute('autoplay', 'autoplay');
			audioElement.play();
		},
		playSoundError  : function () {
			var audioElement = document.createElement('audio');
			audioElement.setAttribute('src', App.getDomain() + 'assets/global/sounds/error.mp3');
			audioElement.setAttribute('autoplay', 'autoplay');
			audioElement.play();
		},
		getToken: function () {
