...
// Set width for inputs in horizontal search bar -----------------------------------------------------------------------
	...
	$("#redefine-search-form").load("assets/tmpl/external/_search-bar.html", function () {
		setInputsWidth();
		//autoComplete();
	});
...
function autoComplete() {
	if (!$("script[src='assets/tmpl/js/leaflet.js']").length) {
...
function drawOwlCarousel(_rtl) {
	$.getScript("assets/tmpl/js/owl.carousel.min.js", function (data, textStatus, jqxhr) {
		...
		if (category == 'real_estate') {
			if (json.data[a].item_specific) {
				if (json.data[a].item_specific.bedrooms) {
					itemSpecific += '<span title="Bedrooms"><img src="assets/tmpl/img/bedrooms.png">' + json.data[a].item_specific.bedrooms + '</span>';
				}
				if (json.data[a].item_specific.bathrooms) {
					itemSpecific += '<span title="Bathrooms"><img src="assets/tmpl/img/bathrooms.png">' + json.data[a].item_specific.bathrooms + '</span>';
				}
				if (json.data[a].item_specific.area) {
					itemSpecific += '<span title="Area"><img src="assets/tmpl/img/area.png">' + json.data[a].item_specific.area + '<sup>2</sup></span>';
				}
				if (json.data[a].item_specific.garages) {
					itemSpecific += '<span title="Garages"><img src="assets/tmpl/img/garages.png">' + json.data[a].item_specific.garages + '</span>';
				}
				return itemSpecific;
			}
		}
...
function quickView(id, url) {
	$.ajax({
		type   : 'POST',
		url    : url,
		data   : {id:id, _token:$('body').attr('token')},
