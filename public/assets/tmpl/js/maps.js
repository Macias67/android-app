...
function createHomepageGoogleMap(_latitude, _longitude, json) {
	$.get("assets/tmpl/external/_infobox.js", function () {
		gMap();
	});
	function gMap() {
		...
		for (var i = 0; i < json.data.length; i++) {
			...
			var infoboxOptions = {
				...
				closeBoxURL           : "assets/tmpl/img/close.png",
		...
		var clusterStyles = [
			{
				url   : 'assets/tmpl/img/cluster.png',
				height: 34,
				width : 34
			}
		];
...
function multiChoice(sameLatitude, sameLongitude, json) {
	...
	$('.modal-window').load('assets/tmpl/external/_modal-multichoice.html', function () {
